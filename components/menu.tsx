...
const Menu = memo(function Menu_(props: {
  t: number,
  x1: number,
  y1: number,
  h: number,
    ...
    if (state.current.hover[1]) {
      if (state.current.hover[0]) {
        state.current.ease = Math.min(
          EASE_TIME, easeInExpo(w / props.w) * EASE_TIME
        );
      }
      setW(easeOutExpo(state.current.ease / EASE_TIME) * props.w);
    } else {
      if (state.current.hover[0]) {
        state.current.ease = Math.min(
          EASE_TIME, easeInExpo(1 - w / props.w) * EASE_TIME
        );
      }
      setW((1 - easeOutExpo(state.current.ease / EASE_TIME)) * props.w);
    }
